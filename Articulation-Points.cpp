...
void dfs(int u){
   ...
   for(it=adj[u].begin();it!=adj[u].end();++it){
      ...
      if(parent[*it]==-1){
         ...
         // If u is root, it is articulation point only if it has more than 2 children
         if(parent[u]==0 && children>1)
           ap[u]=true;
         // If u is not a root,it is articulation point only if some vertex in its subtree has lower or equal low
         if(parent[u]!=0 && low[*it]>=disc[u])
           ap[u]=true;
      }
